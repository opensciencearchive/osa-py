...
from pockets import PDBIngester, validate_structure
...
class TestValidateStructure:
    def test_valid_structure_passes(self, tmp_path):
        ...
        run_hook(validate_structure, meta=VALID_METADATA, files=tmp_path)

    def test_low_resolution_rejected(self, tmp_path):
        (tmp_path / "structure.cif").write_text("data_test")
        with pytest.raises(Reject, match="Resolution too low"):
            run_hook(
                validate_structure,
                meta={**VALID_METADATA, "resolution": 5.0},
                files=tmp_path,
            )

    def test_missing_cif_rejected(self, tmp_path):
...
class TestIntegration:
    def test_ingester_fetches_records(self):
    ...
    def test_full_convention_pipeline(self):
