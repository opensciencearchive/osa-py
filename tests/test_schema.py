...
class ProteinStructure(MetadataSchema):
    ...
    __schema_id__ = "protein-structure-1"
...
class TestSchemaIdDeclaration:
    def test_subclass_without_schema_id_raises(self) -> None:
        with pytest.raises(TypeError, match="must declare ``__schema_id__``"):

            class NoId(MetadataSchema):
                field: str

    def test_subclass_with_uppercase_id_raises(self) -> None:
        with pytest.raises(TypeError, match="invalid"):

            class BadId(MetadataSchema):
                __schema_id__ = "BadSlug"
                field: str

    def test_subclass_with_leading_digit_raises(self) -> None:
        with pytest.raises(TypeError, match="invalid"):

            class BadId(MetadataSchema):
                __schema_id__ = "3d-scan"
                field: str

    def test_subclass_with_underscore_raises(self) -> None:
        with pytest.raises(TypeError, match="invalid"):

            class BadId(MetadataSchema):
                __schema_id__ = "bad_slug"
                field: str

    def test_schema_id_classmethod_returns_slug(self) -> None:
        assert ProteinStructure.schema_id() == "protein-structure-1"
