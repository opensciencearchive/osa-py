...
class TestToFieldDefinitions:
    def test_str_maps_to_text(self) -> None:
        fields = [
            _fd.model_dump(exclude_none=True)
            for _fd in SimpleSchema.to_field_definitions()
        ]
    ...
    def test_date_maps_to_date(self) -> None:
        fields = [
            _fd.model_dump(exclude_none=True)
            for _fd in DateSchema.to_field_definitions()
        ]
    ...
    def test_optional_field_not_required(self) -> None:
        fields = [
            _fd.model_dump(exclude_none=True)
            for _fd in OptionalSchema.to_field_definitions()
        ]
    ...
    def test_unit_in_constraints(self) -> None:
        fields = [
            _fd.model_dump(exclude_none=True)
            for _fd in UnitSchema.to_field_definitions()
        ]
...
class TestFieldMetadataEmission:
    ...
    def test_description_is_emitted(self) -> None:
        fields = [
            _fd.model_dump(exclude_none=True)
            for _fd in DocumentedSchema.to_field_definitions()
        ]
